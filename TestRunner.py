...
class Tester(TestCase):
    ...
    def benchmark(self, person: str="hanjoondev", target: str="",
                        iters: int=100, verbose=True, big_data=False) -> None:
        dt = 'data' if not big_data else 'data_big'
        iters = 3 if big_data and iters == 100 else iters
        p = dirname(__file__)
        files = lambda folder: ldir(join(p, folder))
        bases = sorted([f.strip('.py') for f in files(f'{person}/acmicpc')
                        if f.endswith('.py') 
                        and f.startswith(target if target else 'A')])
        if big_data:
            bases = sorted({f[:6] for f in files(f'{dt}')})
        d = {b: {'module': imodule(f'{person}.acmicpc.{b}')} for b in bases}
        for k, v in d.items():
